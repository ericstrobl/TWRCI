...
load("IHD_whole_blood_exp_SNP_data_controlled2.RData")
gene_names =gsub("\\..*","",colnames(ge_f))
colnames(ge_f) = gene_names

# keep = remove_multicollinear(SNP_data_f)
out = get_sig_genes_ctrl_Xsplit_SPCR_corr_col(SNP_data_f,cbind(ge_f,target_f),leads_f$p,one.sided=FALSE) # qvalue fails when one.sided is TRUE because NULL distribution not well estimated, so switching to one.sided=FALSE
...
EnsID = mapIds(org.Hs.eg.db,
               keys=bb$kg.sets$`hsa05417 Lipid and atherosclerosis`, 
               column="ENSEMBL",
               keytype="ENTREZID",
               multiVals="first")
