my_skeleton_p <- function (suffStat, p, SNPs=NULL, alpha=0.01, Gest = NULL, order=NULL, indepTest=earth_wrap, m.max=Inf) 
{
  if (!is.null(Gest)){
    G = Gest
  } else{
  ...
  # for (i in 1:(p-1)){
  #   if (indepTest(i, p, NULL, NULL, suffStat) >= alpha){
  #     G[i,] = G[,i] = FALSE
  #   }
  # }
  # G[p,]=FALSE # so G[,p]=TRUE
